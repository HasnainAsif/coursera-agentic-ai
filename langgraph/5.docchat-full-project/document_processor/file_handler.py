...
class DocumentProcessor:
    ...
    def process(self, files: List) -> List:
        ...
        all_chunks = []
        seen_hashes = set() # hashes of all chunks
        
        for file in files:
            try:
                # Generate content-based hash for caching
                with open(file.name, "rb") as f:
                    file_hash = self._generate_hash(f.read()) # Generate unique hash based on file content
                
                cache_path = self.cache_dir / f"{file_hash}.pkl" # .pkl is used to save ML models or preprocessed datasets/documents
                if self._is_cache_valid(cache_path):
                    # If file exist in cache, then load it from cache
    ...
    def _process_file(self, file) -> List:
        ...
        markdown = converter.convert(file.name).document.export_to_markdown() # Read content from document(like pdf) and then convert it to markdown
        splitter = MarkdownHeaderTextSplitter(self.headers) # Split markdown based on headers
