...
# Absolute path of Project root directory (current working directory)
BASE_DIR = Path.cwd()
...
class DocumentGeneratorSchema(BaseModel):
    ...
    """
    file_path: str # path of existing file
    name: str # new file name to create for documentation
...
def get_path(relative_path: str) -> Path:
    ...
    """
    try:
        resolved = Path(relative_path).resolve()  # resolve to absolute path - returned path: BASE_DIR/relative_path
        resolved.relative_to(BASE_DIR)  # raises ValueError if outside BASE_DIR. like if relative_path is ../asd.py, then ValueError will be thrown
        return resolved
    except ValueError:
...
async def write_file(file_path: str, content: str, ctx: Context) -> str:
    ...
    try:
        ...
        path.parent.mkdir(parents=True, exist_ok=True) # create parent dirs if missing (e.g. "a/b/c.js" → creates a/b/), ignore if already exist
        ...
        with open(path, "w", encoding='utf-8') as f:
            for i in range(0, total, chunk_size):
                ...
                await ctx.report_progress(progress=written, total=total, message=f"Writing progress: {written}/{total}") # context is used for showing progress
                time.sleep(0.05)  # artificial delay to make progress visible in client UI (remove in production)

        await ctx.report_progress(progress=total, total=total, message="Write complete")
        await ctx.info(f"File written successfully to: {file_path}") # context is used for logging
        return f"File written successfully to: {file_path}"
    except Exception as e:
        await ctx.error(f"Error creating file: {str(e)}") # context is used for logging
        raise
...
async def delete_file(file_path: str, ctx: Context) -> str:
    ...
    try:
        ...
        if path.is_file():
            path.unlink()
            await ctx.info(f"Successfully deleted file {file_path}") # context is used for logging
            return f"Successfully deleted file {file_path}"
        elif path.is_dir():
            await ctx.warning(f"Error: {file_path} is a directory, not a file") # context is used for logging
            return f"Error: {file_path} is a directory, not a file"
        else:
            await ctx.warning(f"File not found: {file_path}") # context is used for logging
            return f"File not found: {file_path}"
    except Exception as e:
        await ctx.error(f"Error deleting file: {str(e)}") # context is used for logging
...
# Check Heading "MCP Resources" in guide.ipynb for a clean breakdown with examples.

# "file:///" has 3 slashes because file:// expects a host (file://<host>/<path>).
# Since we're serving local files, host is empty → file:///server.py, file:///notes.txt, etc.
...
async def read_file_resource(file_name: str) -> dict:
    ...
    Returns:
        JSON string with file_content key, or error key if invalid
    """
    try:
        ...
        if not path.exists() or not path.is_file():
            return json.dumps({"error": f"{file_name} is not a valid file"})
        # Return as JSON string — FastMCP resources must return str, not dict
        return json.dumps({"file_content": path.read_text(encoding='utf-8')})
...
async def list_files_resource() -> dict:
    ...
    try:
        ...
        for item in path.iterdir():
            ...
            items.append({
                "name": item.name,
                "path": str(item.relative_to(BASE_DIR)), # extract relative path WRT. base directory
...
# PROMPTS
# ============================================================================
# Prompts are reusable message templates with parameters — user picks a prompt,
# fills in the parameters, and gets a ready-made message to send to the LLM.
# Think of them as "guided workflows": code_review(file) → structured review prompt.
# Unlike tools (server-controlled), prompts are user-controlled — the user decides
# when and how to use them.

# pass .py, .js or .ts file for crafting prompt to review code on this file.
@mcp.prompt()
async def code_review(file_path: str, ctx: Context) -> str:
    """Generate a prompt for code review and quality evaluation.

    Reads a code file and generates a prompt for LLM to perform a
...
# For More expalination on Elicitation, check Heading "User Elicitation" from guide.ipynb

# Elicitation(Human-In-The-Loop): pauses execution and asks the user for input via the client UI.
# ctx.elicit() sends a message + a schema (DocumentGeneratorSchema) → client renders a form →
# user fills it → result.data holds the response. Requires elicitation handler in the client.
@mcp.prompt()
